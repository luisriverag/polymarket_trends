...
def update_market_history(markets):
    ...
    for market in markets:
        ...
        if m_id not in history:
            history[m_id] = {"question": market.get("question", ""), "slug": market.get("slug", ""), "first_seen": now, "end_date": end_date}
...
def fetch_markets():
    ...
    try:
        ...
        all_new_markets = sorted(all_markets, key=lambda m: m.get("createdAt", ""), reverse=True)[:1000]
        ...
        events = fetch_with_retry(f"{GAMMA_API}/events", {"active": "true", "closed": "false", "limit": 100}) or []
...
def fetch_leaderboard():
    try:
        data = fetch_with_retry(f"{DATA_API}/leaderboard", {"limit": 50})
    ...
    for market in closed_markets + active_markets:
        try:
            ...
            if is_resolved and outcome and m_id not in history:
                history[m_id] = {"question": question, "slug": slug, "outcome": outcome, "end_date": end_date}
